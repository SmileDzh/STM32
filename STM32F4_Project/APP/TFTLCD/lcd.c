...
void LCD_DrawLineWithLen(u16 x1, u16 y1, u16 x2, u16 y2,int len)
{
	u16 t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
...
//显示一个汉字
void displayCharacter(u16 x,u16 y,u8 *cn)
{
	u8 i, j, wordNum;
	u16 color;
	u16 x0=x; 
	...
	while (*cn != '\0')
	{
		...
		{	//wordNum扫描字库的字数
			if ((hanZi[wordNum].Index[0]==*cn)
			     &&(hanZi[wordNum].Index[1]==*(cn+1)))
			{
				for(i=0; i<116; i++) 
				{	//MSK的位数
					color=hanZi[wordNum].Msk[i];
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
